...
public class SearchProjectsHandler : IRequestHandler<SearchProjectsQuery, PagedResult<ProjectOutput>>
{
    private readonly IAppDbContext _db;
    private readonly ICurrentUserService _currentUser;

    public SearchProjectsHandler(IAppDbContext db, ICurrentUserService currentUser)
    {
        _db = db;
        _currentUser = currentUser;
    }

    public async Task<PagedResult<ProjectOutput>> Handle(SearchProjectsQuery request, CancellationToken ct)
    {
        ...
        var query = _db.Projects
            ...
            .Where(p => request.CreatedFrom == null || p.CreatedAt >= request.CreatedFrom)
            .Where(p => request.CreatedTo == null || p.CreatedAt <= request.CreatedTo);
